...
Ravi is a Claude-powered bot with multi-channel messaging. The daemon connects to external NATS and omni services, manages agent sessions via the Claude Agent SDK, and coordinates background runners for cron jobs, heartbeat, outbound campaigns, and event triggers.
