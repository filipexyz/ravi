...
| `RAVI_RUNTIME_SESSION_POOL_MAX` | `60` | Maximum number of live runtime sessions this daemon can keep active at once. Saturated pools queue cold starts and checkpoint wakes until a slot is released |
...
| `effort` | `none` \| `minimal` \| `low` \| `medium` \| `high` \| `xhigh` \| `max` \| `ultra` | `xhigh` | Reasoning effort default used after task/profile and session overrides |
...
ravi instances delete <name> --execute
ravi instances restore <name>
```

`instances delete` is dry-run by default: without `--execute` it prints the plan and exits 3 without deleting.
...
ravi instances routes remove <instance> <pattern> --execute
ravi instances routes restore <instance> <pattern>
```

`routes remove` is dry-run by default: without `--execute` it prints the plan and exits 3 without removing.
...
```bash
ravi instances set vendas dmPolicy pairing
ravi instances set vendas groupPolicy closed
```
