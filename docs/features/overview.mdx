---
title: Features
description: Complete reference for Ravi's automation features — heartbeat, cron, triggers, TTS, image generation, video analysis, transcription, spec mode, ephemeral sessions, cross-session messaging, reactions, and media pipeline.
---
...
# Manual trigger (ignores schedule; dry-run without --execute)
...
ravi cron rm <id> --execute       # dry-run without --execute
```
...
Precedence is `!` before `&&` before `||`. If the filter is empty or missing, the trigger always fires. CLI commands reject invalid filter syntax before persisting it. Persisted filters that do not compile fail closed: the daemon does not load the trigger, logs an error, and `ravi triggers list` shows it as `invalid_filter`. If the data path doesn't exist, the filter returns false (no match).
...
ravi triggers test <id> --execute # Fire with synthetic event data
ravi triggers rm <id> --execute   # dry-run without --execute
```
...
# Delete immediately
ravi sessions delete <session> --execute
```
...
# Bounded computed recap for another agent
ravi sessions recap <session> --json
```
