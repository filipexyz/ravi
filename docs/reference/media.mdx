...
ravi audio generate "Voice message content" --send --execute

# With custom caption when sending
ravi audio generate "Important update" --send --caption "Listen to this" --execute
...
✓ Audio saved: /tmp/ravi-audio-1709123456789.mp3
  Send to chat: ravi media send "/tmp/ravi-audio-1709123456789.mp3" --execute
...
Generate images from text prompts using the configured image provider. Ravi supports Gemini and OpenAI image models.
...
| `RAVI_IMAGE_PROVIDER` | No | Global env default provider (`gemini` or `openai`) |
| `RAVI_IMAGE_MODEL` | No | Global env default model |
...
# OpenAI Image2
ravi image generate "a diagram of Ravi image generation" --provider openai --model gpt-image-2
...
ravi image generate "coffee shop interior" --send --execute
...
✓ Image saved: /tmp/ravi-image-1709123456789.png
  Send to chat: ravi media send "/tmp/ravi-image-1709123456789.png" --execute
...
Analyze videos from YouTube URLs or local files. YouTube uses subtitles/captions first via `yt-dlp` for fast transcripts and falls back to Google Gemini when subtitles are unavailable or analysis is forced. Local files use Gemini. Extracts title, duration, transcript, and when Gemini is used, summary, topics, and timestamped visual descriptions.
...
YouTube subtitle extraction requires `yt-dlp` in the runtime environment and does not require `GEMINI_API_KEY` when subtitles are found.
...
# Force Gemini analysis instead of the subtitles-first path
ravi video analyze "https://www.youtube.com/watch?v=VIDEO_ID" --strategy gemini

# Require subtitles and fail instead of falling back to Gemini
...
- **Strategy** -- `subtitles` or `gemini`
- **Summary** -- 2-3 paragraph comprehensive summary when Gemini is used
- **Topics** -- Bullet list of main topics covered when Gemini is used
- **Transcript** -- Complete transcription of spoken content with speaker labels
- **Chapters** -- YouTube chapters when available
- **Visual Description** -- Timestamped descriptions of visual content (`[MM:SS-MM:SS]` format) when Gemini is used
...
`ravi media send` is synchronous and uses `omni send` under the hood, so it returns a real delivery ack/error instead of only queueing `ravi.media.send`.

### CLI Usage

```bash
# Send from a chat context (channel auto-detected)
ravi media send "/tmp/ravi-image-123.png" --execute

# With caption
ravi media send "/tmp/photo.jpg" --caption "Check this out" --execute

# Send as voice note (audio only)
ravi media send "/tmp/recording.ogg" --ptt --execute
...
| `--channel <channel>` | Optional channel hint for Ravi output/context |
| `--to <chatId>` | Target chat ID |
| `--account <id>` | Ravi account / instance alias |
...
Ravi has an official typed sticker catalog for channels that explicitly support stickers. Initial outbound support is WhatsApp only.
