...
| Topic | Payload |
|-------|---------|
| `ravi.session.{name}.prompt` | `{ prompt, source?, context?, _agentId? }` |
...
Standalone CLI commands that are not tied to a session use `_cli` as the session token:

```
ravi._cli.cli.{group}.{command}
```
