...
A recap is a bounded, computed projection of one session. Another agent can call it to learn what happened without dumping the transcript or reading agent-scoped `MEMORY.md`.

```bash
# Compact human recap
ravi sessions recap <name>

# Structured payload for another agent
ravi sessions recap <name> --json

# Bound the recent user/assistant tail (default: 8, max: 40)
ravi sessions recap <name> -n 12 --json
```

v0 is computed on read from the session row, `session_goals` if present, and the same recent history used by `sessions read`. Empty `summary` / `pinned` / `decisions` fields mean those sources do not exist yet — the command does not invent narrative. Persist-on-compact and session search are out of this slice.

Authorization matches `sessions read` (`access session:<id>`). A chat attach is not permission to recap.
...
```bash
# Set reasoning effort for a specific session
ravi sessions set-effort <name> ultra

# Options: none, minimal, low, medium, high, xhigh, max, ultra, clear
ravi sessions set-effort <name> clear
```

Effort is independent from `thinking`. It takes effect on the next turn; an active runtime is restarted when the effective effort changes. Runtime precedence is task/dispatch/profile override, then session effort override, then agent effort default, then the runtime default `xhigh`.
...
- `sessions recap` returns "not found" for inaccessible sessions
