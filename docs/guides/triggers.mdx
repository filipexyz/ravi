...
| Pattern | Description |
|---------|-------------|
| `ravi._cli.cli.*.*` | CLI command audit events |
...
ravi.*.cli.contacts.*   # use ravi._cli.cli.contacts.*
...
```bash
nats sub "ravi._cli.cli.contacts.*" --server nats://127.0.0.1:4223
```
