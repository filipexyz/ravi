...
`ravi triggers add --filter ...` and `ravi triggers set <id> filter ...` reject invalid syntax before saving it.

Filters that are already persisted but do not compile (for example, written by an older version or by code that bypassed the CLI) **fail closed**: the daemon does not load the trigger, logs an error with the parse error, and the trigger never fires. A broken filter never widens matching, which matters most for `--shell` triggers. `ravi triggers list` shows these triggers with `STATE: invalid_filter` plus a warning, and `ravi triggers show <id>` prints the parse error. Fix the filter or clear it, and the runner activates the trigger on the next refresh:

```bash
ravi triggers set <id> filter 'data.number == "42"'   # fix
ravi triggers set <id> filter -                       # clear (fires on every event)
```
...
Without `--execute` this is a dry-run (exit 3); nothing is deleted:
...
Verify that the trigger shows `ENABLED: yes`, `STATE: active`, and that the topic pattern is correct. The `STATE` column (`runtimeState` in `--json`) is the same verdict the daemon uses when it subscribes:

| State | Meaning |
|-------|---------|
| `active` | Enabled and loaded by the runner |
| `disabled` | Turned off with `ravi triggers disable` |
| `invalid_filter` | The persisted filter does not compile, so the runner does not load the trigger (fail-closed). `filterError` has the parse error. |
| `blocked_topic` | Subscribes to an internal `ravi.session.*` subject, which the runner skips to prevent loops |
...
- `"Skipping trigger with invalid filter (fail-closed); fix or clear the filter to activate it"` — the persisted filter does not compile, so the trigger is not loaded and never fires
