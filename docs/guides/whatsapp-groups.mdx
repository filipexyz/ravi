...
Every external mutation (`create`, `add`, `remove`, `promote`, `demote`, `revoke-invite`, `join`, `leave`, `rename`, `description`, `settings`) is **dry-run by default**: without `--execute` the command exits 3 and only prints the plan — nothing reaches WhatsApp. Re-run with `--execute` to perform the write. Read-only commands (`list`, `info`, `invite`) take no `--execute`.
...
```bash
ravi whatsapp group create "Team Marketing" "5511999999999,5511888888888" --execute
```
...
```bash
ravi whatsapp group create "Finance Team" "5511999999999" --agent financas --execute
```
...
```bash
ravi whatsapp group create "Ad-hoc Group" "5511999999999" --execute
...
ravi whatsapp group create "New Project" "5511999999999,5511888888888" --execute
```
...
```bash
ravi whatsapp group add group:120363425628305127 "5511999999999,5511888888888" --execute
```
...
```bash
ravi whatsapp group remove group:120363425628305127 "5511999999999" --execute
```

### Promoting to Admin

```bash
ravi whatsapp group promote group:120363425628305127 "5511999999999" --execute
```

### Demoting from Admin

```bash
ravi whatsapp group demote group:120363425628305127 "5511999999999" --execute
```
...
ravi whatsapp group settings group:120363425628305127 announcement --execute

# Everyone can send messages
ravi whatsapp group settings group:120363425628305127 not_announcement --execute
```
...
ravi whatsapp group settings group:120363425628305127 locked --execute

# Everyone can edit group info
ravi whatsapp group settings group:120363425628305127 unlocked --execute
```

### Renaming a Group

```bash
ravi whatsapp group rename group:120363425628305127 "New Group Name" --execute
```

### Updating the Description

```bash
ravi whatsapp group description group:120363425628305127 "Updated description for the group" --execute
```

## Leaving a Group

```bash
ravi whatsapp group leave group:120363425628305127 --execute
```
...
```bash
ravi whatsapp group create "Support Team" "5511999999999" --agent suporte --execute
```
...
ravi whatsapp group create "Sales Team" "5511999999999" --account business --agent vendas --execute

# Manage members on a specific account
ravi whatsapp group add group:120363425628305127 "5511888888888" --account business --execute
```
...
ravi whatsapp group create "Project Alpha" "5511999999999,5511888888888" --agent main --execute

# 3. Lock down group settings
ravi whatsapp group settings group:120363405113391144 locked --execute
```
...
# Remove a member (dry-run first if you want to inspect the plan)
ravi whatsapp group remove group:120363425628305127 "5511777777777" --execute

# Add a replacement
ravi whatsapp group add group:120363425628305127 "5511666666666" --execute

# Promote the new member to admin
ravi whatsapp group promote group:120363425628305127 "5511666666666" --execute
```
