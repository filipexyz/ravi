...
Risky external mutations (`create`, `add`, `remove`, `promote`, `revoke-invite`, `join`, `leave`, `rename`, `description`, `settings`) are **dry-run by default**: without `--execute` the command exits 3 and only prints the plan — nothing reaches WhatsApp. Re-run with `--execute` to perform the write. `demote` reduces authority and executes immediately. Read-only commands (`list`, `info`, `invite`) take no `--execute`.
