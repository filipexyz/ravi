...
For agent jobs, `lastStatus`, `lastError`, and `lastDurationMs` describe the agent turn, not the prompt dispatch. When the prompt is published, `lastRunAt` and `nextRunAt` advance and the outcome fields are cleared; they are filled in once the runtime reports the turn as complete (`ok`), failed, or interrupted (`error`, with the runtime error in `lastError`). A run still in progress therefore shows no status. If publishing the prompt itself fails, the job is marked `error` immediately.
...
Run a job immediately, ignoring its schedule. Without `--execute` this is a dry-run (exit 3) that shows the resolved job and the message that would fire:

```bash
ravi cron run <id>            # dry-run: shows the plan, fires nothing
ravi cron run <id> --execute  # actually triggers the job
```
...
5. **Manual test:** `ravi cron run <id> --execute` -- triggers immediately to verify the job works (without `--execute` it only shows the dry-run plan)
