...
```bash
ravi instances pending reject main 5511999999999 --execute
```

Rejecting removes the entry from the pending list without allowing it. Without `--execute` the command is a dry-run: it prints the plan and exits 3 without removing anything.
...
# Remove a route (soft-delete; without --execute it is a dry-run, exit 3)
ravi instances routes remove main "5511*" --execute
...
```bash
# Soft-delete an instance (without --execute it is a dry-run, exit 3)
ravi instances delete vendas --execute
