...
ravi contacts block abc123def456 --execute
```
...
```bash
# Run once without --execute to preview the merge plan (dry-run, exit 3),
# then confirm with --execute.
ravi contacts merge <source-id> <target-id> --execute
```
...
# Remove a contact entirely (without --execute it is a dry-run, exit 3)
ravi contacts remove abc123def456 --execute
```
...
ravi contacts allow abc123def456
ravi contacts block abc123def456 --execute   # without --execute it is a dry-run (exit 3)
```
...
ravi contacts merge <source-id> <target-id> --execute   # without --execute it is a dry-run (exit 3)
```
