...
ravi daemon logs --clear --execute            # Flush PM2 logs (without --execute it is a dry-run, exit 3)
...
ravi agents delete <id> --execute             # Delete an agent (without --execute: dry-run, exit 3)
ravi agents set <id> <key> <value>            # Set property (name, cwd, model, effort, dmScope, mode, defaults, ...)
...
ravi agents reset <id> [nameOrKey|all] --execute  # Reset session (omit for main, "all" for all; without --execute: dry-run, exit 3)
...
ravi instances delete <name>                  # Soft-delete instance immediately
...
ravi instances routes remove <name> <pattern>            # Soft-delete route immediately
...
ravi instances pending reject <name> <contact> --execute  # Reject and remove pending contact (without --execute: dry-run, exit 3)
```
...
ravi contacts remove <contact> --execute      # Remove a contact (without --execute it is a dry-run, exit 3)
ravi contacts allow <contact>                 # Allow a contact
ravi contacts block <contact>                 # Block a contact immediately
...
ravi contacts merge <source> <target> --execute  # Merge contacts (move identities, delete source; without --execute it is a dry-run, exit 3)
```
...
ravi sessions set-effort <nameOrKey> <level>      # Set effort (none..ultra, clear)
ravi sessions set-thinking <nameOrKey> <level>    # Set thinking level (off, normal, verbose, clear)
ravi sessions reset <nameOrKey> --execute         # Reset session (fresh start)
ravi sessions delete <nameOrKey> --execute        # Delete session permanently
...
```bash
ravi settings list                            # List live settings; legacy account.* is hidden by default
...
ravi settings delete <key>                    # Dry-run: shows the delete plan (exit 3)
ravi settings delete <key> --execute          # Actually delete the setting
```
...
ravi cron run <id> --execute                  # Manually trigger a job (dry-run without --execute)
ravi cron rm <id> --execute                   # Delete a job (dry-run without --execute)
```
...
ravi heartbeat trigger <id> --execute         # Manually trigger pending heartbeat work
```
...
ravi triggers test <id> --execute             # Test with fake event data
ravi triggers rm <id> --execute               # Delete trigger (dry-run without --execute)
```
...
ravi whatsapp.group create <name> <participants> --execute   # Create group (comma-separated phones)
  --account <id>                                             #   WhatsApp account
  --agent <id>                                               #   Auto-route to agent
ravi whatsapp.group add <groupId> <participants> --execute     # Add participants
ravi whatsapp.group remove <groupId> <participants> --execute  # Remove participants
ravi whatsapp.group promote <groupId> <participants> --execute # Promote to admin
ravi whatsapp.group demote <groupId> <participants> --execute  # Demote from admin
ravi whatsapp.group invite <groupId>                         # Get invite link (read-only)
ravi whatsapp.group revoke-invite <groupId> --execute        # Revoke invite link
ravi whatsapp.group join <code> --execute                    # Join group via invite
ravi whatsapp.group leave <groupId> --execute                # Leave group
ravi whatsapp.group rename <groupId> <name> --execute        # Rename group
ravi whatsapp.group description <groupId> <text> --execute   # Update description
ravi whatsapp.group settings <groupId> <setting> --execute   # Update settings (announcement, not_announcement, locked, unlocked)
```

Every external mutation above is dry-run by default: without `--execute` it exits 3 and only prints the plan (nothing reaches WhatsApp). `list`, `info`, and `invite` are read-only and take no `--execute`.
...
```bash
ravi whatsapp.dm send <contact> <message> --execute  # Send a DM (resolves LID or phone; without --execute it is a dry-run, exit 3)
  --account <id>                              #   WhatsApp account
ravi whatsapp.dm read <contact> --no-ack      # Read locally without sending a receipt
ravi whatsapp.dm read <contact> --execute     # Read and send the default receipt when an inbound MID exists
  --last <n>                                  #   Number of messages (default: 10)
  --account <id>                              #   WhatsApp account
ravi whatsapp.dm ack <contact> <messageId> --execute  # Send a receipt (without --execute it is a dry-run, exit 3)
  --account <id>                              #   WhatsApp account
```

The default `dm read` only requires `--execute` when the selected history contains an inbound MID and would therefore emit a read receipt. Without `--execute`, that case returns the plan and exits 3; when no receipt can be emitted, the read returns directly. Use `--no-ack` for an always read-only path.
...
```bash
ravi media send <filePath> --execute          # Send media file (image, video, audio, document)
