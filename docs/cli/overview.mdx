...
ravi sessions recap <nameOrKey> [-n count]        # Bounded computed recap (default: 8)
```
...
ravi whatsapp.group demote <groupId> <participants>           # Demote from admin immediately
...
Every risky external mutation above is dry-run by default: without `--execute` it exits 3 and only prints the plan (nothing reaches WhatsApp). `demote` reduces authority and executes immediately. `list`, `info`, and `invite` are read-only and take no `--execute`.
...
ravi whatsapp.dm send <contact> <message> --execute  # Send a DM (resolves LID or phone; without --execute it is a dry-run, exit 3)
  --account <id>                              #   WhatsApp account
ravi whatsapp.dm read <contact>               # Read local history; never sends a receipt
...
`dm read` is always read-only and never sends a receipt. To send a blue-tick receipt intentionally, use `dm ack <contact> <messageId> --execute`.
