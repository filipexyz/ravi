...
5. Check idempotency records for `(restart_epoch, session_key)`. `delivery_kind=notice` means crash recovery fenced the resume and only a restart notice was sent; `decision_reason` says why (`unsafe_snapshot`, `missing_snapshot`, `ineligible_snapshot`). No row means nothing was published.
...
- **Restart notice instead of resume, session active inside 1h**: expected when the snapshot was unsafe to replay; the agent reports the restart and waits for instruction.
- **No event at all and a delivery row exists**: bug; a row must only follow a published event.
