...
contacts belong to this permission group" without editing dozens of raw local-grants
tuples.
...
Live tag checks inside authorization would create a second hidden authorization
surface. Every tool gate, CLI command, app router, provider hook, and future
...
- Hard-coded permission groups in agent config: rejected because it splits
  policy from provider-owned grant state and makes audit/explain incomplete.
