...
Follow the [Credential Rotation requirements](SPEC.md#credential-rotation). This
runbook records the operational sequence; it does not define an alternative policy.

1. Open a rotation incident or scheduled-change record without writing either key.
2. Use the approved private credential workflow to create and place the replacement
   in the managed secret record.
3. Start the designated canary according to the SPEC, then record only redacted
   verification evidence in [`CHECKS.md`](CHECKS.md).
4. Complete the prior-credential closeout or replacement-failure containment exactly
