...
5. `context revoke` executes immediately because it reduces authority. Exit
   `3` applies to credential removal: read `error.plan`, then re-run that
   command adding `--execute`. For `context prune`, the
   equivalent is `--apply --confirm prune-contexts`; for
   `cleanup-agent-runtime`, it is `--revoke`.
6. If a revoke asks for `--execute`, or a credential entry removal executes
   without it, the risk policy regressed.
...
Live checks against the local CLI using an isolated `RAVI_STATE_DIR`:

```bash
ravi context info ctx-nope --json                    # expect exit 1 + suggestions
ravi context revoke <context-id> --json              # expect immediate revoke
