...
- Legacy `CloudAuthError` payload errors (e.g. `PAYLOAD_INVALID` when a mailbox
  references an unknown account) keep the pre-existing cloud exit map, which
  can exit 3 for non-brake failures. This predates the agent-first taxonomy and
  is declared debt: contract ops rethrow `ContractError` first, so braked and
  not-found paths are correct; the legacy funnel remains for untyped payload
  errors until the cloud-auth exit map is reconciled.
