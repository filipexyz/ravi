...
4. Provider resolution/validation (no provider configured, `--async`+`--sync`
   ...
   MUST use the canonical JSON envelope in JSON mode; remaining compatibility
   validation paths use `COMMAND_FAILED` in JSON mode, retain their concise
...
| flag conflicts | `COMMAND_FAILED` compatibility envelope | 1 |
