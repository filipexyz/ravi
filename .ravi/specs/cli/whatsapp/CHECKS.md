...
- Every braked op (`group send|create|add|remove|promote|demote|revoke-invite|join|leave|rename|description|settings`, `dm send`, `dm ack` and a default `dm read` with a receipt candidate) invoked without `--execute` MUST exit 3 with `dryRun: true` and a sanitized `plan`, and MUST NOT make any provider/NATS call — not even the group-metadata read on the send path.
...
- `whatsapp group list --fields a,b --json` and `whatsapp dm read <contact> --no-ack --fields a,b --json` MUST return items containing only the requested fields.
