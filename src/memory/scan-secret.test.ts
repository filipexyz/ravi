...
describe("scanSecret (R9b redact-at-source)", () => {
  ...
  it("detects github token and redacts at source", () => {
    const token = ["ghp_", "abcdefghijklmnopqrstuvwxyz", "0123456789"].join("");
  ...
  it("detects openai key", () => {
    const key = ["sk-proj-", "ABCDEFghijkl", "0123456789xyz"].join("");
  ...
  it("detects AWS access key + slack token", () => {
    const content = `AWS=${["AKIA", "IOSFODNN7EXAMPLE"].join("")} and slack=${["xoxb", "1234567890", "abcdef"].join("-")}.`;
    const result = scanSecret(content);
    expect(result.hasSecret).toBe(true);
  ...
  it("flags credential-only when the entire content is one credential", () => {
    const result = scanSecret(["ghp_", "abcdefghijklmnopqrstuvwxyz", "0123456789"].join(""));
  ...
  it('detects generic hardcoded credential (`api_key = "..."`) even without a known prefix', () => {
    const value = ["abcDEF123456", "ghiJKL789xyz"].join("");
    const configLine = `api_key = "${value}"`;
  ...
  it("detects hardcoded password / secret variants with colon or hyphenated key", () => {
    const pwd = scanSecret(`password: "${["SuperSecret", "ValueThatIs22Chars"].join("_")}"`);
    const dashKey = scanSecret(`api-key = "${["01234567890", "ABCDEFGHIJ+/="].join("")}"`);
