...
interface MemorySnapshotOptions {
  ...
  memoryPath?: string;
  /**
   * R13 read-side cap — the prompt-injection budget (`DEFAULT_MEMORY_CAP_CHARS`),
   * DECOUPLED from the write/file cap (memory-lifecycle L1). The MEMORY.md file
   * may grow past this (the fill-triggered lifecycle keeps it healthy); the head
   * injected here is truncated-with-marker at this budget so the per-turn prompt
   * cost stays bounded regardless of file size. Read cap ≤ write cap always, so
   * the read side can never bypass the write side.
   */
  capChars?: number;
}
