...
/**
 * R1c — carry the hook-owned `memoryCuration` cadence state forward when the
 * RUNTIME rewrites `runtimeSessionParams` on its per-turn persist.
 *
 * The runtime's turn.complete persist does a full-column replace of
 * provider-authoritative params (sessionId, skillVisibility, …). The cadence
 * counter, however, is written OUT-OF-BAND by the memory-curator Stop hook via
 * `writeMemoryCurationState` into the same `runtime_session_json` column. If
 * the runtime's replace drops `memoryCuration`, it clobbers the hook's counter
 * on every single turn (classic lost-update on a shared JSON document), so the
 * counter never accumulates past 1 and cadence never fires.
 *
 * This carries the existing `memoryCuration` (the freshest value the runtime
 * read from the DB) forward into whatever params the runtime is about to
 * write, making the runtime a good citizen of the shared column. Provider
 * params never carry `memoryCuration`, so this only ever preserves the hook's
 * state — it does not overwrite provider-authoritative data.
 */
export function preserveMemoryCurationState(
