...
export function isAgentCreatedSkillContent(content: string): boolean {
  return AGENT_CREATED_RE.test(extractFrontmatter(content));
}

export function agentCreatedSkillOwner(content: string): string | undefined {
  const match = extractFrontmatter(content).match(/^\s*created_by:\s*['"]?([^'"\r\n]+)['"]?\s*$/im);
...
export function applySkillGuard(input: SkillGuardInput): SkillGuardDecision {
  ...
  if (input.op === "patch") {
    ...
    if (!isAgentCreatedSkillContent(current)) {
      return {
        outcome: "rejected",
        reason: "protected",
...
export function archiveAgentCreatedSkill(skillName: string, homeDir = homedir(), dryRun = false): SkillArchiveDecision {
  ...
  if (!isAgentCreatedSkillContent(readFileSync(path, "utf-8"))) {
    return {
      outcome: "rejected",
      reason: "protected",
