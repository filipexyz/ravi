...
const SETTINGS_FILE = join(PI_AGENT_DIR, "settings.json");
...
export class PiCommands {
  ...
  status(@Option({ flags: "--json", description: "Print raw JSON result" }) asJson?: boolean) {
    ...
    const managed = existsSync(EXTENSIONS_DIR)
      ? readdirSync(EXTENSIONS_DIR).filter(
          (f) => f.endsWith(".ts") && readFileSync(join(EXTENSIONS_DIR, f), "utf-8").includes(EXTENSION_MARKER),
        )
      : [];
    const payload = {
      piCommand: resolvePiCommand(),
      piVersion: piVersion(),
      defaultProvider: (settings.defaultProvider as string) ?? null,
      defaultModel: (settings.defaultModel as string) ?? null,
  ...
  doctor(
    ...
    if (agentId) {
      ...
      if ((agent!.provider ?? null) !== "pi") {
        issues.push(`agent "${agentId}" runtimeProvider is "${agent!.provider ?? "(default)"}", not "pi"`);
      }
    ...
    const payload = {
      ...
      model,
      provider,
      authenticated: provider ? authed.has(provider) : authed.size > 0,
...
export class PiModelsCommands {
  ...
  add(
    ...
    const extPath = join(EXTENSIONS_DIR, `${providerId}.ts`);
    ...
    const inputArr = (input ?? "text")
      ...
      .map((s) => s.trim())
      .filter(Boolean);
    const ctx = Number(contextWindow ?? 128000) || 128000;
    const maxOut = Number(maxTokens ?? 8192) || 8192;
    const label = name ?? providerId!;
    const source = `// ${EXTENSION_MARKER} — generated by \`ravi pi models add\`. Edit via the CLI, not by hand.
...
export default function (pi: ExtensionAPI) {
  pi.registerProvider(${JSON.stringify(providerId)}, {
    ...
    apiKey: ${JSON.stringify(apiKeyEnv)},
    api: ${JSON.stringify(api ?? "openai-completions")},
    models: [
      {
