...
import * as taskAutomations from "../../tasks/index.js";
...
describe("tasks automations agent-first contract", () => {
  it("minimizes the rm dry-run plan and leaves the automation untouched", () => {
    const deleteAutomation = spyOn(taskAutomations, "deleteTaskAutomation");
    try {
      const automation = taskAutomations.createTaskAutomation({
      ...
      expect(taskAutomations.getTaskAutomation(automation.id)).not.toBeNull();
    } finally {
      deleteAutomation.mockRestore();
    }
  });
});
