...
describe("AdapterCommands", () => {
  ...
  it("lists health, bind, and last event details from persisted debug snapshots", () => {
    saveSessionAdapter({
  ...
  it("shows protocol-invalid explicitly when the persisted snapshot has a protocol failure", () => {
    saveSessionAdapter({
      adapterId: TEST_ADAPTER_ID,
      definition: buildDefinition(),
      status: "broken",
