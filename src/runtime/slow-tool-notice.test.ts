...
describe("slow tool watch config", () => {
  ...
  it("keeps the tick short enough to hold the presence alive", () => {
    ...
    expect(resolveSlowToolWatchConfig({}).tickMs).toBeLessThanOrEqual(resolveSlowToolWatchConfig({}).announceAfterMs);
  });
...
describe("slow tool status text", () => {
  ...
  it("reports the queue without leaking chat noise", () => {
