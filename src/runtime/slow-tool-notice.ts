 ...
 * na conversa — cada tool longa gerava uma mensagem nova, e o agente que roda
 * vários comandos lentos spammava a pessoa. Aviso de estado operacional não é
 * conversa.
 *
 * **O que ele faz:** mantém a sessão visivelmente viva (presença) e publica o
 * estado ao vivo da tool em execução — quanto tempo, qual tool, quantas mensagens
 * esperando. Estado é contínuo e sobrescrito; mensagem é discreta e acumula. A
 * transparência mora no estado.
 *
 * A solução estrutural para chamada longa é outra: promover para background e
 * avisar o *agente*, com o agente no controle (`ravi jobs wait|tail|kill`).
 */
