...
export function shouldInterruptRuntimeForIncoming(
  ...
  if (session.toolRunning) {
    // Interrupt explícito sempre vence. A restrição de tool "unsafe" saiu daqui:
    // ela existia quando abortar uma tool em execução não era possível, e desde
    // que o abort passou a matar o processo, a restrição só servia para deixar a
    // mensagem da pessoa presa atrás de um comando longo.
