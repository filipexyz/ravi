...
export class RaviClient {
  ...
  readonly runtime = {
    continuity: {
      /** Apply a frozen single-agent preview with approval and idempotency */
      apply: async (agentId: string, options?: {
        approvalRef?: string;
        idempotencyKey?: string;
        planHash?: string;
      }): Promise<RuntimeContinuityApplyReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "apply",
          body: { agentId, ...(options ?? {}) },
        });
      },
      /** Apply an immutable batch plan atomically per agent */
      batchApply: async (batchId: string, options?: {
        approvalRef?: string;
        idempotencyKey?: string;
        planHash?: string;
      }): Promise<RuntimeContinuityBatchApplyReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "batch-apply",
          body: { batchId, ...(options ?? {}) },
        });
      },
      /** Freeze an immutable selected-agent or all-agent policy plan */
      batchPreview: async (options?: {
        agents?: string;
        all?: boolean;
        deadlineMs?: string;
        disabled?: boolean;
        targets?: string;
      }): Promise<RuntimeContinuityBatchPreviewReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
      ...
      /** Read a batch preview or apply result */
      batchStatus: async (batchId: string): Promise<RuntimeContinuityBatchStatusReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
      ...
      /** Clear one agent's policy through a versioned preview/apply mutation */
      clear: async (agentId: string, options?: {
        approvalRef?: string;
        expectedVersion?: string;
        idempotencyKey?: string;
      }): Promise<RuntimeContinuityClearReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "clear",
          body: { agentId, ...(options ?? {}) },
      ...
      /** Read the latest persisted decision for a logical request */
      decision: async (logicalRequestId: string): Promise<RuntimeContinuityDecisionReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "decision",
          body: { logicalRequestId },
        });
      },
      /** Explain ordered target eligibility without starting a provider call */
      explain: async (agentId: string): Promise<RuntimeContinuityExplainReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "explain",
          body: { agentId },
      ...
      /** Freeze and validate a single-agent policy plan without applying it */
      preview: async (agentId: string, options?: {
        deadlineMs?: string;
        disabled?: boolean;
        expectedVersion?: string;
        targets?: string;
      }): Promise<RuntimeContinuityPreviewReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "preview",
          body: { agentId, ...(options ?? {}) },
      ...
      /** Resolve an ambiguous effect by stable effect id and evidence */
      reconcile: async (effectId: string, options?: {
        evidenceRef?: string;
        outcome?: string;
      }): Promise<RuntimeContinuityReconcileReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "reconcile",
          body: { effectId, ...(options ?? {}) },
        });
      },
      /** Move one target to another ordered index using compare-and-set */
      reorder: async (agentId: string, fromIndex: number, toIndex: number, options?: {
        approvalRef?: string;
        expectedVersion?: string;
        idempotencyKey?: string;
      }): Promise<RuntimeContinuityReorderReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "reorder",
          body: { agentId, fromIndex, toIndex, ...(options ?? {}) },
        });
      },
      /** Resume a safe non-terminal journal from its durable snapshot */
      resume: async (logicalRequestId: string): Promise<RuntimeContinuityResumeReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "resume",
          body: { logicalRequestId },
      ...
      /** Preview and apply an ordered policy in one compare-and-set operation */
      set: async (agentId: string, options?: {
        approvalRef?: string;
        deadlineMs?: string;
        disabled?: boolean;
        expectedVersion?: string;
        idempotencyKey?: string;
        targets?: string;
      }): Promise<RuntimeContinuitySetReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "set",
          body: { agentId, ...(options ?? {}) },
      ...
      /** Show one agent's continuity policy and target recovery state */
      show: async (agentId: string): Promise<RuntimeContinuityShowReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "show",
          body: { agentId },
      ...
      /** Replay redacted continuity decisions with cursor pagination */
      trace: async (logicalRequestId: string, options?: {
        cursor?: string;
        limit?: string;
      }): Promise<RuntimeContinuityTraceReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "trace",
          body: { logicalRequestId, ...(options ?? {}) },
        });
      },
      /** Persist a bounded recovery wait before a journal deadline */
      wait: async (logicalRequestId: string, options?: {
        until?: string;
      }): Promise<RuntimeContinuityWaitReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "wait",
          body: { logicalRequestId, ...(options ?? {}) },
      ...
      /** Re-read and wake a due durable journal */
      wake: async (logicalRequestId: string): Promise<RuntimeContinuityWakeReturn> => {
        return this.transport.call({
          groupSegments: ["runtime","continuity"],
          command: "wake",
          body: { logicalRequestId },
