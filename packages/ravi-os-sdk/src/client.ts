...
export class RaviClient {
  ...
  readonly context = {
    ...
    /** Evaluate a Codex PreToolUse native tool hook payload from stdin using the current Ravi context */
    codexToolHook: async (): Promise<ContextCodexToolHookReturn> => {
      return this.transport.call({
        groupSegments: ["context"],
        command: "codex-tool-hook",
        body: {},
      });
    },
    credentials: {
    ...
    /** Issue a profile-scoped root context for a local extension or long-lived CLI */
    issueRoot: async (cliName: string, options?: {
      agent?: string;
      kind?: string;
      label?: string;
      profile?: string;
      ...
      skipSave?: boolean;
      ttl?: string;
    }): Promise<ContextIssueRootReturn> => {
      return this.transport.call({
        groupSegments: ["context"],
        command: "issue-root",
        body: { cliName, ...(options ?? {}) },
